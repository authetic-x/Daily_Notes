...
> React 会集齐一堆需要更新的组件然后一起更新，从而保证渲染的性能。具体细节日后再仔细研究，这里给出两个经典场景：
>
> 1. 如果后续逻辑需要最新的 state，可以在 componentDidUpdate 或者 setState 的回调函数里去做
> 2. 连续在一个地方更新多次 state，可以给 steState 传入函数形式：`this.setState((prevState, props) => {})`

使用 HOC 写一个 loading 组件
...
> 多个路由使用相同组件时，组件只会重新 render，不会重新构建。如果想每次完整的触发组件的生命周期，可以在 `Route` 中加上 key